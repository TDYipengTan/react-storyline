...
describe("<Flex />", () => {
  ...
  it("renders with a ref", () => {
    ...
    const { getByTestId } = render(
      <Flex forwardedRef={ref} data-testid="flex" />
    );
  ...
  it("renders with custom classes", () => {
    const { getByTestId } = render(
      <Flex className="custom-styles" data-testid="flex" />
    );
  ...
  it("passes its direction to the children and applies the right class", () => {
    const { getByTestId } = render(
    ...
    expect(getByTestId("one").parentElement!.classList).toContain(
      "co-flex--column"
    );
    expect(getByTestId("two").parentElement!.classList).toContain(
      "co-flex--column"
    );
  });

  it("passes its horizontal alignment to the children and applies the right class", () => {
    const { getByTestId } = render(
  ...
  it("passes its vertical alignment to the children and applies the right class", () => {
    const { getByTestId } = render(
  ...
  it("supports a custom width based on the viewport", () => {
    const { getByTestId } = render(
      <Flex width={[100, 200, 300]} data-testid="flex" />
    );
    const node = getByTestId("flex");
  ...
  it("supports a custom height based on the viewport", () => {
    const { getByTestId } = render(
      <Flex height={[100, 200, 300]} data-testid="flex" />
    );
    const node = getByTestId("flex");
  ...
  it("applies both horizontal and vertical padding based on the viewport", () => {
    const { getByTestId } = render(
      <Flex padding={[1, 2, 3]} data-testid="flex" />
    );
    const node = getByTestId("flex");
  ...
  it("applies horizontal padding based on the viewport", () => {
    const { getByTestId } = render(
      <Flex paddingX={[1, 2, 3]} data-testid="flex" />
    );
    const node = getByTestId("flex");
  ...
  it("applies vertical padding based on the viewport", () => {
    const { getByTestId } = render(
      <Flex paddingY={[1, 2, 3]} data-testid="flex" />
    );
    const node = getByTestId("flex");
  ...
  describe("when there is a gap", () => {
    ...
    it("makes the outer container scrollable", () => {
      const { getByTestId } = render(
        <Flex gap={2} scrollable data-testid="flex" />
      );
      const node = getByTestId("flex").parentElement!;
  ...
  describe("When direction is set to row", () => {
    it("aligns items horizontally with the corresponding spacing properties", () => {
      ...
      expect(flexSpaceAroundX.classList).toContain(
        "co-flex--align-x-space-around"
      );
      expect(flexSpaceBetweenX.classList).toContain(
        "co-flex--align-x-space-between"
      );
      expect(flexSpaceEvenlyX.classList).toContain(
        "co-flex--align-x-space-evenly"
      );
    });
  ...
  describe("When direction is set to column", () => {
    ...
    it("aligns items vertically with the spacing properties", () => {
      ...
      expect(flexSpaceAroundY.classList).toContain(
        "co-flex--align-y-space-around"
      );
      expect(flexSpaceBetweenY.classList).toContain(
        "co-flex--align-y-space-between"
      );
      expect(flexSpaceEvenlyY.classList).toContain(
        "co-flex--align-y-space-evenly"
      );
    });
    ...
    it("alignY defaults to center when the stretch property is passed", () => {
      const { getByTestId } = render(
        ...
        <Flex
          direction="column"
          alignY="stretch"
          data-testid="flex-default-center-y"
        />
      );

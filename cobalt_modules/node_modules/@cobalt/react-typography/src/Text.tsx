...
type AlignProps =
  | {
      inline: true;
      /** This prop must not be used when `inline` is true */
      textAlign?: never;
    }
  | {
...
type Props = CommonProps & AlignProps;
...
const Skeleton: FC<{
  ...
  const classStyles = cc([
    styles["co-text"],
    size && styles[classNames[getSizeKey(size)]],
  ...
  if (lines > 1 && !inline) {
    return (
      <Tag {...props} ref={forwardedRef} style={{ width, margin: 0 }}>
...
const Text: FC<Props> = ({
  ...
  const classStyles = [
    styles["co-text"],
    size && styles[classNames[getSizeKey(size)]],
    weight && styles[classNames[getWeightKey(weight)]],
    {
